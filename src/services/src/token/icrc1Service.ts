...
import ICRC1, { MetadataValue } from "./interface/icrc1";

export class ICRC1TokenService implements ITokenService {
  public readonly actor: ICRC1;
  public readonly decimals: number = 1e8;

  constructor({ actor, decimals }: { actor: ICRC1; decimals?: number }) {
    this.actor = actor;
    if (decimals) {
      this.decimals = 1 * Math.pow(10, decimals);
    }
  }
  async approve(input: { fee: [bigint] | []; memo: [] | [Uint8Array | number[]]; from_subaccount: [] | [Uint8Array | number[]]; created_at_time: [bigint] | []; amount: bigint; expected_allowance: [bigint] | []; expires_at: [bigint] | []; spender: { owner: Principal; subaccount: [] | [Uint8Array | number[]]; }; }): Promise<bigint> {
