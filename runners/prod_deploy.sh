...
dfx canister install token --ic --mode update --argument "(opt record {icrc1 = opt record {
  name = opt \"$TOKEN_NAME\";
...
dfx canister call token admin_init --ic
...
dfx canister call token icrc1_decimals  --query --ic
dfx canister call token icrc1_fee  --query --ic
dfx canister call token icrc1_metadata  --query --ic
